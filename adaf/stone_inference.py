...
def semantic_segmentation_vectors(predictions_dirs_dict, threshold=0.5,
    ...
    for label, predicts_dir in predictions_dirs_dict.items():
        ...
        for file in tif_list:
            with rasterio.open(file) as src:
                ...
                # Remove edge by 8 pixels  # todo: remove buffer before saving prob mask (prob not, mask saved in ML)
                re = 8
            ...
            if poly:
                ...
                # if keep_ml_paths: #todo remove from code?
    ...
    if gdf_out:
        ...
        gdf["area"] = gdf.geometry.area.round(1)
...
def run_visualisations(dem_path, tile_size, save_dir, nr_processes=1):
    ...
    # todo valid_data_outline, _ = gt.poly_from_valid(in_file.as_posix())
    ...
    # todo tiles_extents = gt.filter_by_outline(tiles_extents, valid_data_outline)
...
if __name__ == "__main__":
    dem_path = Path(r"r:\BiH_ALS_2025\BiH_ALS_2025_DMO_05m.vrt")
    out_dir = Path(r"r:\ML podatki\ml_results")
    custom_model_pth = Path(r"r:\ML podatki\models\BiH_v5_run4.pth.tar")
    vis_path = Path(r"r:\delovno\nejc\stone_visualisations\BiH_ALS_2025_DMO_05m_slrm4inference")

    # Other params:
    ml_type = "segmentation"
    labels = ["custom"]
    ml_model_custom = "Custom model"
    save_ml_output = False  # Probability maps
    min_area = 0
    roundness = 0
    ...
    # Create logfile #TODO log other params
    log_path = save_dir / "logfile.txt"
    logger = Logger(log_path, log_time=time_started)

    logger.log_inference_inputs(ml_type, labels, ml_model_custom, custom_model_pth)
    # For logger
    save_raw = []
    t2 = time.time()

    logging.debug("Running segmentation")
    predictions_dict = run_aitlas_segmentation(labels, vis_path, custom_model_pth)

    vector_path = semantic_segmentation_vectors(
        predictions_dict,
        keep_ml_paths=save_ml_output,
        roundness=roundness,
        min_area=min_area
    )
    if vector_path != "":
    ...
    # Log inference results (roundness not used for obj. detection)
    logger.log_inference_results(vector_path, t2, save_raw, min_area, roundness)
